...
const fallbackEs: EsRow[] = [
  { id: "fallback-1", user_id: null, title: "SaaS PM インターン ES", content_md: "", questions: null, status: "draft", tags: ["SaaS"], score: null, ai_summary: null, company_name: "Alpha SaaS", company_url: null, selection_status: "書類提出", memo: null, created_at: null, updated_at: null },
  { id: "fallback-2", user_id: null, title: "マーケ職向け ES", content_md: "", questions: null, status: "submitted", tags: ["Marketing"], score: null, ai_summary: null, company_name: "Sky Finance", company_url: null, selection_status: "面接", memo: null, created_at: null, updated_at: null },
];

const fallbackCompanies: CompanyRow[] = [
  { id: "fallback-c1", user_id: null, name: "Alpha SaaS", url: "alphasaas.jp", memo: null, stage: "Screening", preference: 3, favorite: false, ai_summary: null, created_at: null, updated_at: null },
  { id: "fallback-c2", user_id: null, name: "Sky Finance", url: "skyfin.co.jp", memo: null, stage: "Document passed", preference: 4, favorite: false, ai_summary: null, created_at: null, updated_at: null },
];
    ...
    return (
      <AppLayout headerActions={headerActions} className="flex flex-col gap-10">
        <section className="rounded-3xl border border-white/70 bg-white/80 p-8 shadow-xl backdrop-blur dark:border-slate-700/70 dark:bg-slate-900/80">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-4">
              ...
              <p className="max-w-2xl text-sm leading-7 text-slate-700">ESドラフト・企業管理・AIフィードバックを一か所に集約し、XPで進捗を可視化。毎週のモチベーション維持を支援します。</p>
            </div>
            <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white/90 p-5 text-sm shadow-soft backdrop-blur">
              ...
              <div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100">
                <div className="h-full rounded-full bg-linear-to-r from-amber-400 to-rose-400" style={{ width: `${Math.min((data.xp / XP_NEXT_LEVEL) * 100, 100)}%` }} aria-hidden />
              </div>
              <div className="mt-4">
                <SignOutButton />
              </div>
        ...
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {kpis.map((item) => (
            <div key={item.title} className="h-full rounded-2xl border border-white/70 bg-white/80 p-4 shadow-md backdrop-blur dark:border-slate-700/70 dark:bg-slate-900/80">
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          ...
          <article className="rounded-2xl border border-white/70 bg-white/80 p-5 shadow-md backdrop-blur lg:col-span-2 dark:border-slate-700/70 dark:bg-slate-900/80">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              {(data.esEntries.length ? data.esEntries : fallbackEs).map((es) => (
                <Link key={es.id} href={`/es/${es.id}`} className="block h-full">
                  <div className="h-full rounded-xl border border-slate-200 bg-white px-4 py-4 shadow transition-all hover:-translate-y-1 hover:shadow-md">
                    <p className="text-xs font-semibold text-slate-600">企業名</p>
                    <p className="text-sm font-semibold text-slate-900">{es.company_name || "未設定"}</p>
                    <p className="mt-2 text-xs font-semibold text-slate-600">ESタイトル</p>
                    <p className="text-sm text-slate-900">{es.title}</p>
                    <div className="mt-3 flex items-center justify-between text-xs text-slate-700">
                      ...
                      <span className="text-slate-500">更新: {es.updated_at ? new Date(es.updated_at).toLocaleDateString() : "-"}</span>
                    </div>
                    {es.tags?.length ? (
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          <article className="rounded-2xl border border-white/70 bg-white/80 p-5 shadow-md backdrop-blur lg:col-span-2 dark:border-slate-700/70 dark:bg-slate-900/80">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {(data.companies.length ? data.companies : fallbackCompanies).map((company) => (
                <Link key={company.id} href={`/companies/${company.id}`} className="block h-full">
                  <div className="h-full rounded-xl border border-slate-200 bg-white px-4 py-4 shadow transition-all hover:shadow-md">
                    <p className="text-sm font-semibold text-slate-900">{company.name}</p>
                    <div className="mt-3 flex flex-wrap gap-2 text-[11px]">
                      <span className="rounded-full bg-slate-100 px-3 py-1 text-slate-800">ステータス: {company.stage || "未設定"}</span>
                      <span className="rounded-full bg-amber-50 px-3 py-1 text-amber-800">志望度: {company.preference ?? "-"}</span>
                    </div>
                  </div>
                </Link>
              ))}
          ...
          <article className="rounded-2xl border border-white/70 bg-white/80 p-5 shadow-md backdrop-blur dark:border-slate-700/70 dark:bg-slate-900/80">
            ...
            <ul className="mt-4 space-y-3 text-sm text-slate-800">
              {aiQueue.map((item) => (
                <li key={item.title} className="h-full rounded-xl border border-slate-200 bg-white px-3 py-3 shadow">
